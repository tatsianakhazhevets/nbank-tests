...
import apiTests.iteration2_senior.steps.DataBaseStep;
...
import static apiTests.iteration2_senior.specs.Messages.UNAUTHORIZED_ACCESS_TO_ACCOUNT;

@APIVersion("with_database_with_fix")
public class DepositMoneySeniorTestWithDatabase extends BaseTest {
    @ValueSource(doubles = {
    ...
    public void authorizedUserDepositsMoneySuccessfully(double deposit) {
        ...
        UserAccountsResponse retrieveAccountsResponse = AccountCheckStep.retrieveAccount(userAccounts, accountId);
        TransactionNestedResponse userTransferDeposit = AccountCheckStep.getTransferDeposit(retrieveAccountsResponse);

        TransactionDao transactionDao = DataBaseStep.getTransactionByAccountId(accountId.getId());
        DaoAssertingClass.assertThat(userTransferDeposit, transactionDao).match();
        softly.assertThat(retrieveAccountsResponse.getId()).isEqualTo(transactionDao.getAccountId());
    }


    public static Stream<Arguments> depositInvalidCases() {
        return Stream.of(
                Arguments.of(5000.01, DEPOSIT_AMOUNT_CANNOT_EXCEED_5000_LIMIT.getMessage()), // Authorized user cannot deposit 5000.01 (T4_Negative)
                Arguments.of(0.00, INVALID_ACCOUNT_OR_AMOUNT.getMessage()) // Deposit amount must be positive – test with 0,00 (T7_Negative)
        );
    }
    ...
    public void authorizedUserCannotDepositInvalidAmount(double deposit, String errorMessage) {
        ...
        List<TransactionDao> transactionsDao = DataBaseStep.getTransactionsByAccountId(accountId.getId());
        softly.assertThat(transactionsDao).isEmpty();
    }
    ...
    public void authorizedUserCannotDepositToNonExistingAccount() {
        ...
        List<TransactionDao> transactionsDao = DataBaseStep.getTransactionsByAccountId(depositMoneyRequest.getId());
        softly.assertThat(transactionsDao).isEmpty();
    }
    ...
    public void authorizedUserCannotDepositToAnotherUsersAccount() {
        ...
        List<TransactionDao> transactionsDao = DataBaseStep.getTransactionsByAccountId(secondUserAccountId.getId());
        softly.assertThat(transactionsDao).isEmpty();
    }
}