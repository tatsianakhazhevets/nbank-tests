...
import apiTests.iteration2_senior.assertions.DaoAssertingClass;
...
import apiTests.iteration2_senior.steps.*;
...
public class TransferMoneySeniorTestWithDatabase extends BaseTest {
    ...
    public void userTransfersMoneyBetweenOwnAccountsSuccessfully(double deposit, double transferAmount) {
        ...
        softly.assertThat(retrieveFirstAccountsResponse.getBalance()).isCloseTo(((deposit + deposit) - transferAmount), within(0.001));
        ...
        List<TransactionDao> transactionsDaoFirstAccount = DataBaseStep.getTransactionsByAccountId(firstAccountId.getId());
        TransactionDao transactionDaoFirstUser = transactionsDaoFirstAccount.stream()
                .filter(t -> apiTests.iteration2_senior.models.TransactionType.TRANSFER_OUT.getType().equals(t.getType()))
                .findFirst()
                .orElseThrow();
        DaoAssertingClass.assertThat(userTransferOutAmount, transactionDaoFirstUser).match();
        softly.assertThat(retrieveFirstAccountsResponse.getId()).isEqualTo(transactionsDaoFirstAccount.get(0).getAccountId());

        TransactionDao transactionDaoSecondAccount = DataBaseStep.getTransactionByAccountId(secondAccountId.getId());
        DaoAssertingClass.assertThat(userTransferInAmount, transactionDaoSecondAccount).match();
        softly.assertThat(retrieveSecondAccountsResponse.getId()).isEqualTo(transactionDaoSecondAccount.getAccountId());
    }
    ...
    public static Stream<Arguments> invalidTransfersBetweenOwnAccounts() {
        return Stream.of(
                ...
                Arguments.of(50, 0.00, INVALID_TRANSFER_INSUFFICIENT_FUNDS_OR_INVALID_ACCOUNT.getMessage()), // Transfer amount must be positive – test with 0.00 within first user accounts (T21_Negative test)
    ...
    public void userCannotTransferInvalidAmountsBetweenAccounts(double deposit, double transferAmount, String errorMessage) {
        ...
        List<TransactionDao> transactionsDaoFirstAccount = DataBaseStep.getTransactionsByAccountId(firstAccountId.getId());
        softly.assertThat(transactionsDaoFirstAccount)
                .noneMatch(t -> TransactionType.TRANSFER_OUT.getType().equals(t.getType()));
        List<TransactionDao> transactionDaoSecondAccount = DataBaseStep.getTransactionsByAccountId(secondAccountId.getId());
        softly.assertThat(transactionDaoSecondAccount)
                .noneMatch(t -> TransactionType.TRANSFER_IN.getType().equals(t.getType()));
    }
    ...
    public void userCanTransferMoneyToAnotherUserSuccessfully(double deposit, double transferAmount) {
        ...
        softly.assertThat(userTransferInAmount.getAmount()).isEqualTo(transferAmount);
        softly.assertThat(userTransferOutAmount.getAmount()).isEqualTo(transferAmount);
        softly.assertThat(retrieveFirstUserAccountsResponse.getBalance()).isCloseTo(((deposit + deposit) - transferAmount), within(0.001));
        softly.assertThat(retrieveSecondUserAccountsResponse.getBalance()).isEqualTo(transferAmount);
        softly.assertThat(transferMoneyResponse.getMessage()).isEqualTo(TRANSFER_SUCCESSFUL.getMessage());

        List<TransactionDao> transactionsDaoFirstUser = DataBaseStep.getTransactionsByAccountId(firstUserAccountId.getId());
        TransactionDao transactionDaoFirstUser = transactionsDaoFirstUser.stream()
                .filter(t -> apiTests.iteration2_senior.models.TransactionType.TRANSFER_OUT.getType().equals(t.getType()))
                .findFirst()
                .orElseThrow();
        softly.assertThat(retrieveFirstUserAccountsResponse.getId()).isEqualTo(transactionsDaoFirstUser.get(0).getAccountId());
        DaoAssertingClass.assertThat(userTransferOutAmount, transactionDaoFirstUser).match();
        TransactionDao transactionDaoSecondUser = DataBaseStep.getTransactionByAccountId(secondUserAccountId.getId());
        DaoAssertingClass.assertThat(userTransferInAmount, transactionDaoSecondUser).match();
        softly.assertThat(retrieveSecondUserAccountsResponse.getId()).isEqualTo(transactionDaoSecondUser.getAccountId());
    }
    ...
    public static Stream<Arguments> invalidTransferToAnotherUser() {
        return Stream.of(
                ...
                Arguments.of(200, 0.00, INVALID_TRANSFER_INSUFFICIENT_FUNDS_OR_INVALID_ACCOUNT.getMessage()), // Transfer amount must be positive – test with 0.00 among accounts of the different users (T22_Negative test)
    ...
    public void userCannotTransferMoneyToAnotherUser(double deposit, double transferAmount, String errorMessage) {
        ...
        List<TransactionDao> transactionsDaoFirstUser = DataBaseStep.getTransactionsByAccountId(firstUserAccountId.getId());
        softly.assertThat(transactionsDaoFirstUser)
                .noneMatch(t -> TransactionType.TRANSFER_OUT.getType().equals(t.getType()));
        List<TransactionDao> transactionDaoSecondUser = DataBaseStep.getTransactionsByAccountId(secondUserAccountId.getId());
        softly.assertThat(transactionDaoSecondUser)
                .noneMatch(t -> TransactionType.TRANSFER_IN.getType().equals(t.getType()));
    }
}